...
level_test_parallel <- function(gen_fun,
  ...
  clusterExport(
    ...
    c("gen_fun","perm_test_metric", "perm_test_mardia",
      ...
      'mardia',"u1_statistic","u2_statistic", "metric_skew_asym",
  ...
  for (i in seq_along(sample_sizes)) {
    ...
    results_hhat_asym[i]   <- mean(pvals[2, ] < alpha)
    results_mardia[i] <- mean(pvals[3, ] < alpha)
    results_mardia_asym[i] <- mean(pvals[4, ] < alpha)
    
    
    cat("Done n =", n,
        "| metric skew perm:", round(results_hhat[i], 3),
        "| mteric skew asym:", round(results_hhat_asym[i], 3),
